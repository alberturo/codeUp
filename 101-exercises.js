...
function firstToLast(sequence) {
  const firstValue = sequence.shift();
...
function sumAll(sequenceNum) {
  const sum = sequenceNum.reduce((acc, currValue) => {
...
function mean(sequence) {
  const mean = sumAll(sequence) / sequence.length;
  return mean;
}
...
function median(sequence) {
  const sortedNumbers = sequence.slice().sort((a, b) => a - b);
...
function productOfAll(sequence) {
  const product = sequence.reduce((acc, curr) => acc * curr, 1);
  return product;
}
...
function onlyPositiveEvens(array) {
  let positivesEvens = [];
  for (let i = 0; i < array.length; i++) {
    array[i] % 2 === 0 && array[i] > 0 && positivesEvens.push(array[i]);
  }
  return positivesEvens;
}
...
function onlyPositiveOdds(array) {
  let positiveOdds = [];
  for (let i = 0; i < array.length; i++) {
    array[i] > 0 && array[i] % 2 !== 0 && positiveOdds.push(array[i]);
  }
  return positiveOdds;
}
...
function onlyNegativeEvens(array) {
  let negativeEvens = [];
  for (let i = 0; i < array.length; i++) {
    array[i] < 0 && array[i] % 2 === 0 && negativeEvens.push(array[i]);
  }
...
function onlyNegativeOdds(array) {
  let negativeOdds = [];
  for (let i = 0; i < array.length; i++) {
    array[i] < 0 && array[i] % 2 !== 0 && negativeOdds.push(array[i]);
  }
  return negativeOdds;
}
...
function flatten(array) {
  const flatten = array.reduce((flatArr, subArr) => flatArr.concat(subArr));
  return flatten;
}
...
function getAverageBookPrice(obj) {
  return totalOfBookPrices(obj) / obj.length;
}
...
function lowestPriceBook(obj) {
  ...
  let lowestPriceBook = obj[0];

  for (let i = 1; i < obj.length; i++) {
    if (obj[i].price < lowestPriceBook.price) {
      lowestPriceBook = obj[i];
    }
  }
  return {
    title: lowestPriceBook.title,
    author: lowestPriceBook.author,
    price: lowestPriceBook.price,
  };
}
...
function numberOfItemTypes({ items }) {
  let itemTypes = [];
  for (const item of items) {
    if (!itemTypes.includes(item.title)) {
      itemTypes.push(item.title);
    }
  }
...
function totalNumberOfItems({ items }) {
  let totalItemsQty = 0;
  for (let i = 0; i < items.length; i++) {
    totalItemsQty += items[i].quantity;
  }
...
function getAverageItemPrice(shoppingCart) {
  ...
  const averagePrice = totalPrice / numberOfItemTypes(shoppingCart);
  return averagePrice;
}
...
function getAverageSpentPerItem({ items }) {
  let totalPrice = 0;
  let totalQuantity = 0;
  let totalSpent = 0;
  for (let i = 0; i < items.length; i++) {
    ...
    totalSpent += items[i].price * items[i].quantity;
  }
  const avergageSpent = totalSpent / totalQuantity;
  return avergageSpent;
}
...
function mostSpentOnItem({ items }) {
  let mostSpentItem = {
    title: "",
    price: 0,
    quantity: 0,
  ...
  for (let i = 0; i < items.length; i++) {
    if (
      ...
      mostSpentItem.price * mostSpentItem.quantity
    ) {
      mostSpentItem = {
        title: items[i].title,
        price: items[i].price,
